    ...
    <div className="p-6 max-w-[1400px] mx-auto h-[calc(100vh-100px)] flex flex-col">
      ...
      <div className="flex-1 grid lg:grid-cols-3 gap-8 min-h-0">
        ...
        <div className="lg:col-span-2 flex flex-col rounded-3xl border border-white/5 bg-[#1E293B] shadow-2xl overflow-hidden relative">
          <div className="absolute top-0 right-0 h-[500px] w-[500px] bg-cyan-500/5 rounded-full blur-3xl -mr-40 -mt-40 pointer-events-none" />
                ...
                <div className="grid grid-cols-1 gap-2 w-full max-w-md">
                  {SUGGESTED_QUESTIONS.map((q) => (
                    <button
                      ...
                      disabled={loading}
                      className="text-left text-sm px-4 py-3 rounded-xl border border-white/10 bg-[#0F172A] text-slate-300 hover:border-cyan-500/40 hover:text-white hover:bg-cyan-500/5 transition-all duration-200 flex items-center gap-3"
                    >
              ...
              <>
                ...
                {loading && (
                  <div className="flex items-start">
                    <div className="max-w-[80%] rounded-2xl rounded-tl-sm p-4 bg-[#0F172A] border border-white/5">
                      <div className="flex items-center gap-2 mb-2 text-cyan-400">
                        <Sparkles className="h-4 w-4" />
                        <span className="text-xs font-bold uppercase tracking-widest">WalletSathi AI</span>
                      </div>
            ...
            <form onSubmit={handleSend} className="relative flex items-center">
              ...
              <div className="absolute right-2 flex items-center gap-1">
                ...
                <button
                  ...
                  className="p-2 bg-cyan-500 text-slate-900 rounded-full hover:bg-cyan-400 transition disabled:opacity-50 disabled:bg-slate-600 flex items-center justify-center"
                >
        ...
        <div className="space-y-6 overflow-y-auto pr-2 scrollbar-hide hidden lg:block">
          ...
          <div className="rounded-2xl border border-cyan-500/20 bg-[#1E293B] p-5 relative overflow-hidden group">
            <div className="absolute right-0 top-0 h-24 w-24 bg-cyan-500/10 rounded-full blur-xl -mr-10 -mt-10 transition group-hover:bg-cyan-500/20" />
            ...
            <div className="space-y-2 relative z-10">
              {SUGGESTED_QUESTIONS.slice(0, 4).map((q) => (
                <button
                  ...
                  onClick={() => handleSuggestion(q)}
                  disabled={loading}
                  className="block w-full text-left text-xs text-slate-400 hover:text-cyan-400 transition py-1 border-b border-white/5 last:border-0"
                >
                  → {q}
                </button>
              ))}
            ...
            <div className="h-35 flex items-end gap-2 pt-1">
              {[25, 70, 30, 90, 60, 80].map((height, i) => (
                <div key={i} className="flex-1 flex flex-col items-center gap-2 group cursor-pointer">
                  <div className="w-full bg-[#0F172A] rounded-t-sm relative" style={{ height: "128px" }}>
                    <div
                      className={`w-full rounded-t-sm transition-all duration-500 absolute bottom-0 ${i === 3 ? "bg-cyan-500" : "bg-cyan-500/20 group-hover:bg-cyan-500/40"
                        }`}
                      style={{ height: `${height}%` }}
                    />
                  </div>
