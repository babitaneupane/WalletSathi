  ...
  return (
    <div className="p-6 max-w-[1400px] mx-auto h-[calc(100vh-100px)] flex flex-col">
      ...
      <div className="flex-1 grid lg:grid-cols-3 gap-8 min-h-0">
        ...
        <div className="lg:col-span-2 flex flex-col rounded-3xl border border-white/5 bg-[#1A1333] shadow-2xl overflow-hidden relative">
          <div className="absolute top-0 right-0 h-[500px] w-[500px] bg-indigo-500/5 rounded-full blur-3xl -mr-40 -mt-40 pointer-events-none" />
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-6 relative z-10 scrollbar-hide">
            {historyLoading ? (
              ...
              <div className="flex flex-col items-center justify-center h-full gap-6 text-center px-4">
                ...
                <div>
                  <h2 className="text-xl font-bold text-white mb-2">Ask me about your finances</h2>
                ...
                <div className="grid grid-cols-1 gap-2 w-full max-w-md">
                  {SUGGESTED_QUESTIONS.map((q) => (
                    <button
                      ...
                      disabled={loading}
                      className="text-left text-sm px-4 py-3 rounded-xl border border-white/10 bg-[#0F0B1E] text-slate-300 hover:border-indigo-500/40 hover:text-white hover:bg-indigo-500/5 transition-all duration-200 flex items-center gap-3"
                    >
              ...
              <>
                <div className="flex justify-center my-2">
                  <span className="text-[10px] font-bold px-3 py-1 bg-[#0F0B1E] text-slate-500 rounded-full uppercase tracking-widest border border-white/5">
                  ...
                  <div key={msg.id} className={`flex flex-col ${msg.role === "user" ? "items-end" : "items-start"}`}>
                    <div
                      className={`max-w-[80%] rounded-2xl p-4 ${msg.role === "user"
                        ? "bg-indigo-500 text-slate-900 rounded-tr-sm"
                        : "bg-[#0F0B1E] border border-white/5 text-slate-200 rounded-tl-sm"
                        }`}
                    >
                      {msg.role === "assistant" && (
                ...
                {loading && (
                  <div className="flex items-start">
                    <div className="max-w-[80%] rounded-2xl rounded-tl-sm p-4 bg-[#0F0B1E] border border-white/5">
                      <div className="flex items-center gap-2 mb-2 text-indigo-400">
                        <Sparkles className="h-4 w-4" />
                        <span className="text-xs font-bold uppercase tracking-widest">WalletSathi AI</span>
                      </div>
          ...
          {/* Input Area */}
          <div className="p-6 bg-[#1A1333] border-t border-white/5 relative z-10 shrink-0">
            <form onSubmit={handleSend} className="relative flex items-center">
              ...
              <input
                ...
                className="w-full rounded-full border border-white/10 bg-[#0F0B1E] py-4 pl-12 pr-24 text-sm text-white placeholder:text-slate-500 focus:border-indigo-500/50 focus:outline-none focus:ring-1 focus:ring-indigo-500/30 transition shadow-inner"
              />
              <div className="absolute right-2 flex items-center gap-1">
                ...
                <button
                  ...
                  className="p-2 bg-indigo-500 text-slate-900 rounded-full hover:bg-indigo-400 transition disabled:opacity-50 disabled:bg-slate-600 flex items-center justify-center"
                >
        ...
        <div className="space-y-6 overflow-y-auto pr-2 scrollbar-hide hidden lg:block">
          ...
          <div className="rounded-2xl border border-indigo-500/20 bg-[#1A1333] p-5 relative overflow-hidden group">
            <div className="absolute right-0 top-0 h-24 w-24 bg-indigo-500/10 rounded-full blur-xl -mr-10 -mt-10 transition group-hover:bg-indigo-500/20" />
            <div className="flex items-center gap-3 mb-3 relative z-10">
              <div className="h-8 w-8 rounded-full bg-indigo-500/10 flex items-center justify-center">
                <Sparkles className="h-4 w-4 text-indigo-400" />
              </div>
              <h4 className="font-bold text-white text-sm">Try Asking</h4>
            </div>
            <div className="space-y-2 relative z-10">
              {SUGGESTED_QUESTIONS.slice(0, 4).map((q) => (
                <button
                  ...
                  onClick={() => handleSuggestion(q)}
                  disabled={loading}
                  className="block w-full text-left text-xs text-slate-400 hover:text-indigo-400 transition py-1 border-b border-white/5 last:border-0"
                >
                  → {q}
                </button>
              ))}
          ...
          {/* Budget Alert Card */}
          <div className="rounded-2xl border border-rose-500/20 bg-[#1A1333] p-5 relative overflow-hidden group">
            ...
            <div className="flex items-center gap-3 mb-3 relative z-10">
              ...
              <h4 className="font-bold text-white text-sm">Tip</h4>
            </div>
            <p className="text-xs text-slate-400 leading-relaxed mb-4 relative z-10">
              Ask the AI about <strong className="text-white">specific date ranges</strong> like &quot;How much did I spend last week?&quot; for precise analysis.
            </p>
          </div>
            ...
            <div className="h-35 flex items-end gap-2 pt-1">
              {[25, 70, 30, 90, 60, 80].map((height, i) => (
                <div key={i} className="flex-1 flex flex-col items-center gap-2 group cursor-pointer">
                  <div className="w-full bg-[#0F0B1E] rounded-t-sm relative" style={{ height: "128px" }}>
                    <div
                      className={`w-full rounded-t-sm transition-all duration-500 absolute bottom-0 ${i === 3 ? "bg-indigo-500" : "bg-indigo-500/20 group-hover:bg-indigo-500/40"
                        }`}
                      style={{ height: `${height}%` }}
                    />
                  </div>
