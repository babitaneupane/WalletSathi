...
const CATEGORY_COLORS: Record<string, string> = {
  ...
  Transport: 'text-emerald-500 bg-indigo-50',
    ...
    <div className="space-y-6 p-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <h1 className="text-2xl font-bold text-slate-900 mt-0.5" style={{ fontFamily: "var(--font-outfit)" }}>Budgets & Analytics</h1>
        </div>
        <button
          onClick={() => setIsModalOpen(true)}
          className="flex items-center gap-2 rounded-xl bg-emerald-500 hover:bg-emerald-500 px-4 py-2.5 text-sm font-semibold text-slate-900 shadow-lg shadow-emerald-500/25 transition-all hover:scale-105 active:scale-95 shrink-0"
        >
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-16 flex flex-col items-center justify-center text-center gap-4">
          <div className="rounded-full bg-emerald-500/10 p-4">
            <Plus className="h-8 w-8 text-emerald-500" />
          </div>
          <h3 className="text-lg font-bold text-slate-800" style={{ fontFamily: "var(--font-outfit)" }}>No budgets yet</h3>
          <p className="text-slate-500 text-sm max-w-xs">Set spending limits for each category to track where your money goes.</p>
          <button
            onClick={() => setIsModalOpen(true)}
            className="rounded-xl bg-emerald-500 hover:bg-emerald-500 px-5 py-2.5 text-sm font-semibold text-slate-900 shadow-lg shadow-emerald-500/25 transition"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {budgets.map((budget: any) => {
            ...
            return (
              <div key={budget.id} className="rounded-2xl border border-slate-200 bg-white p-5 flex flex-col hover:border-slate-200 transition">
                <div className="flex justify-between items-start mb-5">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="font-bold text-slate-800" style={{ fontFamily: "var(--font-outfit)" }}>{categoryName}</h3>
                  ...
                  <div className="flex gap-2 items-center">
                    ...
                    <div className="relative w-12 h-12">
                      <svg className="w-12 h-12 transform -rotate-90">
                        <circle cx="24" cy="24" r="20" stroke="currentColor" strokeWidth="3" fill="transparent" className="text-slate-900/5" />
                        <circle cx="24" cy="24" r="20" stroke="currentColor" strokeWidth="3" fill="transparent"
                          ...
                          className={isNearLimit ? "text-red-400" : "text-emerald-500"}
                          style={{ transition: "stroke-dashoffset 0.5s ease" }}
                        />
                      </svg>
                      <div className="absolute top-0 left-0 w-full h-full flex items-center justify-center text-[9px] font-bold text-slate-700">
                ...
                <div className="space-y-2">
                  <div className="flex justify-between text-xs">
                    ...
                    <span className="font-bold text-slate-800">NPR {spent.toLocaleString()}</span>
                  </div>
                  <div className="h-1.5 w-full rounded-full bg-slate-50 overflow-hidden">
                    <div
                      className={`h-full rounded-full transition-all duration-700 ${isNearLimit ? "bg-red-400" : "bg-gradient-to-r from-emerald-500 to-emerald-500"}`}
        ...
        >
          <div className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-6 shadow-2xl">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-lg font-bold text-slate-900" style={{ fontFamily: "var(--font-outfit)" }}>Add New Budget</h2>
              <button onClick={() => setIsModalOpen(false)} className="text-slate-500 hover:text-slate-700 transition">
                <X className="h-5 w-5" />
              </button>
            </div>
            <form onSubmit={handleCreateBudget} className="space-y-4">
              <div>
                ...
                <input type="number" required value={formData.amount}
                  ...
                  className="w-full rounded-xl border border-slate-200 bg-slate-50 py-3 px-4 text-sm text-slate-800 placeholder:text-slate-600 focus:border-emerald-500/50 focus:outline-none transition"
                  placeholder="0.00"
                />
              </div>
              <div>
              ...
              <button type="submit"
                className="w-full rounded-xl bg-emerald-500 hover:bg-emerald-500 py-3 text-sm font-semibold text-slate-900 shadow-lg shadow-emerald-500/25 transition-all mt-2 hover:scale-[1.02] active:scale-[0.98]"
              >
                Save Budget
              </button>
            </form>
