...
export default function HelpPage() {
        ...
        <div className="space-y-3 text-sm text-slate-700 leading-relaxed">
          ...
          <div className="space-y-2">
            <div className="flex items-start gap-2">
              <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-emerald-500/10 text-emerald-500 text-xs font-bold mt-0.5">1</span>
              <p>Click the <strong className="text-slate-900 font-medium">plus (+) icon</strong> in the Savings card on the dashboard to open the modal.</p>
            </div>
            <div className="flex items-start gap-2">
              <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-emerald-500/10 text-emerald-500 text-xs font-bold mt-0.5">2</span>
              <p>Enter the goal name (e.g., "Europe Trip"), target amount, and initial saved amount.</p>
            </div>
            <div className="flex items-start gap-2">
              <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-emerald-500/10 text-emerald-500 text-xs font-bold mt-0.5">3</span>
      ...
      content: (
        <div className="space-y-3 text-sm text-slate-700 leading-relaxed">
          ...
          <div className="space-y-2">
            <div className="flex items-start gap-2">
              <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-emerald-500/10 text-emerald-500 text-xs font-bold mt-0.5">1</span>
              <p>Hover your cursor over the goal card inside the <strong className="text-slate-900 font-medium">Savings</strong> widget on the dashboard.</p>
            </div>
            <div className="flex items-start gap-2">
              <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-emerald-500/10 text-emerald-500 text-xs font-bold mt-0.5">2</span>
              <p>Click the <strong className="text-slate-900 font-medium">Pencil (Edit)</strong> icon to update details, or the <strong className="text-slate-900 font-medium">Trash (Delete)</strong> icon to delete the goal.</p>
            </div>
            <div className="flex items-start gap-2">
              <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-emerald-500/10 text-emerald-500 text-xs font-bold mt-0.5">3</span>
              <p>Inside the edit modal, modify the amounts and click <strong className="text-slate-900 font-medium">Save Changes</strong>.</p>
            </div>
          </div>
        </div>
      ),
        ...
        <div className="space-y-3 text-sm text-slate-700 leading-relaxed">
          ...
          <div className="space-y-2">
            <div className="flex items-start gap-2">
              ...
              <p>Go to the <strong className="text-slate-900 font-medium">Split Expenses</strong> page from the sidebar navigation.</p>
            </div>
            <div className="flex items-start gap-2">
    ...
    },
    {
      ...
      content: (
        <div className="space-y-3 text-sm text-slate-700 leading-relaxed">
          ...
          <p>Navigate to the <strong className="text-slate-900 font-medium">AI Assistant</strong> page to ask queries such as:</p>
  ...
  return (
    <div className="p-6 space-y-6 max-w-4xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row gap-3 items-center justify-between">
        ...
        <div className="relative w-full sm:max-w-xs">
          ...
          <input
            ...
            className="w-full rounded-xl border border-slate-200 bg-slate-50 py-2 pl-10 pr-4 text-sm text-slate-800 placeholder:text-slate-600 focus:border-emerald-500/50 focus:outline-none focus:ring-1 focus:ring-emerald-500/30 transition"
          />
        </div>

        {/* Tabs */}
        <div className="flex gap-1.5 p-1 rounded-xl bg-slate-900/60 border border-slate-200 w-full sm:w-auto overflow-x-auto">
          {(["all", "savings", "split", "ai", "rent"] as const).map((tab) => (
            <button
              ...
              className={`rounded-lg px-3.5 py-1.5 text-xs font-semibold capitalize transition-all ${
                activeTab === tab
                  ? "bg-emerald-500 text-slate-900 shadow-lg shadow-emerald-500/15 font-bold"
        ...
        {filteredArticles.length > 0 ? (
          filteredArticles.map((article) => {
            ...
            return (
              <div
                key={article.id}
                className="group rounded-2xl border border-slate-200 bg-white p-6 shadow-xl transition-all duration-300 hover:border-slate-200 hover:shadow-2xl"
              >
                <div className="flex items-center gap-3.5 mb-4">
                  <div className="p-2 rounded-xl bg-slate-50 text-emerald-500 group-hover:scale-110 transition duration-300">
                    <Icon className="h-5 w-5" />
                  </div>
                  <h2 className="text-lg font-bold text-slate-900 group-hover:text-emerald-500 transition" style={{ fontFamily: "var(--font-outfit)" }}>
                    {article.title}
                  </h2>
                </div>
          ...
          <div className="flex flex-col items-center justify-center py-16 rounded-2xl border border-slate-200 bg-white/40 text-slate-600">
      ...
      <div className="rounded-2xl bg-gradient-to-r from-emerald-500/10 to-purple-600/10 border border-emerald-500/10 p-6 flex flex-col sm:flex-row items-center justify-between gap-4">
        <div>
          <h3 className="font-bold text-slate-900 flex items-center gap-2" style={{ fontFamily: "var(--font-outfit)" }}>
            <Sparkles className="h-4 w-4 text-emerald-500" /> Need personalized help?
          </h3>
        ...
        <a
          href="/ai-assistant"
          className="flex items-center gap-2 rounded-xl bg-emerald-500 hover:bg-emerald-500 px-4 py-2.5 text-sm font-bold text-slate-900 shadow-lg shadow-emerald-500/20 transition-all duration-200 shrink-0"
        >
