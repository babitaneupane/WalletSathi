    ...
    <div className="space-y-6 p-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <h1 className="text-2xl font-bold text-slate-900 mt-0.5" style={{ fontFamily: "var(--font-outfit)" }}>Financial Groups</h1>
          <p className="text-sm text-slate-500 mt-1">Manage shared expenses with friends, family, and teams.</p>
        </div>
        <div className="flex gap-3 shrink-0">
          <button onClick={() => { setError(""); setIsJoinModalOpen(true); }} className="flex items-center gap-2 rounded-xl border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm font-semibold text-slate-700 hover:bg-slate-100 transition">
            <UserPlus className="h-4 w-4" /> Join Group
          </button>
          <button onClick={() => { setError(""); setIsCreateModalOpen(true); }} className="flex items-center gap-2 rounded-xl bg-emerald-500 hover:bg-emerald-500 px-4 py-2.5 text-sm font-bold text-slate-900 shadow-lg shadow-emerald-500/25 transition">
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-16 flex flex-col items-center justify-center text-center gap-4">
          <div className="rounded-full bg-emerald-500/10 p-4"><Users className="h-8 w-8 text-emerald-500" /></div>
          <h3 className="text-lg font-bold text-slate-800" style={{ fontFamily: "var(--font-outfit)" }}>No groups yet</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {groups.map((group: any) => (
            <div key={group.id} className="rounded-2xl border border-slate-200 bg-white p-5 flex flex-col hover:border-slate-200 transition group">
              <div className="flex justify-between items-start mb-4">
                <div className="flex items-center gap-3">
                  <div className="p-2.5 rounded-xl bg-emerald-500/10 text-emerald-500"><Users className="h-5 w-5" /></div>
                  <div>
                    <h3 className="font-bold text-slate-800" style={{ fontFamily: "var(--font-outfit)" }}>{group.name}</h3>
              ...
              <div className="flex -space-x-2 my-3">
                {(group.members || []).slice(0, 4).map((m: any, i: number) => (
                  <div key={i} className="h-8 w-8 rounded-full border-2 border-[#1E293B] bg-slate-700 flex items-center justify-center text-xs font-bold text-slate-900">
                    {(m.user?.name || m.name || "?").charAt(0).toUpperCase()}
                  </div>
                ))}
                {(group.members?.length || 0) > 4 && (
                  <div className="h-8 w-8 rounded-full border-2 border-[#1E293B] bg-emerald-500/20 text-emerald-500 flex items-center justify-center text-xs font-bold">
              ...
              <div className="mt-auto pt-4 border-t border-slate-200 flex justify-between items-center">
                ...
                <button
                  ...
                  className="flex items-center gap-1.5 text-xs font-semibold text-emerald-500 hover:text-teal-500 transition"
                >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: "rgba(0,0,0,0.6)", backdropFilter: "blur(8px)" }}
          ...
          <div className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-6 shadow-2xl">
            ...
            <button
              ...
              className="mt-5 w-full rounded-xl border border-slate-200 py-2.5 text-sm font-medium text-slate-500 hover:bg-slate-50 transition"
            >
              Close
            </button>
          </div>
          ...
          <div className="w-full max-w-sm rounded-2xl border border-slate-200 bg-white p-6 shadow-2xl">
            <div className="flex justify-between items-start mb-4">
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-lg font-bold text-slate-900" style={{ fontFamily: "var(--font-outfit)" }}>Delete Group?</h2>
              </div>
              <button onClick={() => setDeleteModalOpen(false)} disabled={isDeleting} className="text-slate-500 hover:text-slate-700 transition">
                <X className="h-5 w-5" />
              </button>
            </div>
            <p className="text-sm text-slate-500 mb-6">Are you sure you want to delete this group? This action cannot be undone.</p>
            <div className="flex gap-3">
