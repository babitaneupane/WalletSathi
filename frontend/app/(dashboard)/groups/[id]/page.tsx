  ...
  if (!group) {
    return (
      <div className="flex flex-col items-center justify-center h-full p-6 gap-4">
        ...
        <button onClick={() => router.push("/groups")} className="text-emerald-500 hover:underline text-sm font-semibold flex items-center gap-1">
    ...
    <div className="space-y-6 p-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-slate-200 pb-6">
        <div className="flex items-center gap-4">
          <button onClick={() => router.push("/groups")} className="p-2 rounded-xl border border-slate-200 bg-slate-50 text-slate-500 hover:text-slate-900 hover:bg-slate-100 transition">
        ...
        <button onClick={() => setIsExpenseModalOpen(true)} className="flex items-center gap-2 rounded-xl bg-emerald-500 hover:bg-emerald-500 px-4 py-2.5 text-sm font-bold text-slate-900 shadow-lg shadow-emerald-500/25 transition">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="rounded-2xl border border-slate-200 bg-white p-5">
          ...
          <p className="text-2xl font-bold text-slate-900">NPR {totalSpent.toLocaleString()}</p>
        </div>
        <div className="rounded-2xl border border-slate-200 bg-white p-5">
          <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-2">Your Share</p>
          <p className="text-2xl font-bold text-emerald-500">NPR {group.members?.length > 0 ? (totalSpent / group.members.length).toLocaleString(undefined, { maximumFractionDigits: 0 }) : "0"}</p>
        </div>
        <div className="rounded-2xl border border-slate-200 bg-white p-5">
          ...
          <div className="flex -space-x-2 mt-1">
            {(group.members || []).slice(0, 6).map((m: any, i: number) => (
              <div key={i} className="h-8 w-8 rounded-full border-2 border-[#1E293B] bg-slate-700 flex items-center justify-center text-xs font-bold text-slate-900" title={m.user?.name}>
      ...
      {/* Expenses List */}
      <div>
        <h2 className="text-lg font-bold text-slate-900 mb-4" style={{ fontFamily: "var(--font-outfit)" }}>Expense Feed</h2>
        {expenses.length === 0 ? (
          <div className="rounded-2xl border border-slate-200 bg-white p-12 flex flex-col items-center gap-4 text-center">
            <div className="h-12 w-12 rounded-full bg-emerald-500/10 flex items-center justify-center"><Receipt className="h-6 w-6 text-emerald-500" /></div>
            <h3 className="font-bold text-slate-800">No expenses yet</h3>
            <p className="text-sm text-slate-500 max-w-xs">Click "Add Expense" to record a shared expense for this group.</p>
          </div>
        ) : (
          <div className="space-y-3">
            {expenses.map((expense: any) => (
              <div key={expense.id} className="rounded-2xl border border-slate-200 bg-white p-5 flex items-center justify-between hover:border-slate-200 transition">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-1.5 mt-1.5 flex-wrap">
                      {(expense.splits || []).map((split: any, i: number) => (
                        <span key={i} className="text-[10px] font-semibold bg-slate-50 text-slate-500 px-2 py-0.5 rounded-full">
                ...
                <div className="text-right shrink-0">
                  <p className="font-bold text-slate-900 text-lg">NPR {expense.amount.toLocaleString()}</p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: "rgba(0,0,0,0.6)", backdropFilter: "blur(8px)" }}
          ...
          <div className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-6 shadow-2xl">
            <div className="flex justify-between items-center mb-5">
              <div>
                <h2 className="text-lg font-bold text-slate-900" style={{ fontFamily: "var(--font-outfit)" }}>Add Group Expense</h2>
                <p className="text-xs text-slate-500 mt-0.5">Will be split equally among {group.members?.length || 1} member(s)</p>
              </div>
              <button onClick={() => setIsExpenseModalOpen(false)} className="text-slate-500 hover:text-slate-700 transition"><X className="h-5 w-5" /></button>
            </div>
            <form onSubmit={handleAddExpense} className="space-y-4">
              ...
              <div>
                ...
                {expenseForm.amount && group.members?.length > 0 && (
                  <p className="text-xs text-slate-500 mt-1.5">Each person pays: <strong className="text-emerald-500">NPR {(parseFloat(expenseForm.amount) / group.members.length).toLocaleString(undefined, { maximumFractionDigits: 2 })}</strong></p>
                )}
              </div>
              <div className="flex gap-3 pt-1">
                <button type="button" onClick={() => setIsExpenseModalOpen(false)} className="flex-1 rounded-xl border border-slate-200 py-2.5 text-sm font-medium text-slate-500 hover:bg-slate-50 transition">Cancel</button>
